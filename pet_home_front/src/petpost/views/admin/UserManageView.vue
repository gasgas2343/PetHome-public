...
onMounted(async () => {
  if (!authStore.canAccessBackstage && !authStore.isAdmin) {
    alert('你沒有管理員權限')
    router.push('/')
    return
  }
      ...
      <tbody>
        <tr v-for="user in adminUserStore.users" :key="user.userId">
          ...
          <td>
            ...
            <span v-else-if="isSuspendedUser(user)" class="badge text-bg-warning"> 論壇停權 </span>
          ...
          <td>
            ...
            <span v-else class="text-secondary"> 無操作 </span>
          </td>
        </tr>
      </tbody>
    </table>
